...
function SuccessContent() {
  ...
  useEffect(() => {
    if (!orderId) return;
    const storageKey = `cb_px_${orderId}`;
    if (sessionStorage.getItem(storageKey)) return;
    sessionStorage.setItem(storageKey, "1");
    trackPurchase(PRICE, `cb_${orderId}`);
  }, [orderId]);

  useEffect(() => {
    if (seconds <= 0) {
      router.push("/bellezagracias");
      return;
    }
      ...
      <div className="w-full max-w-md text-center space-y-5">
        ...
        <div>
          ...
          <p className="text-gray-500 text-base">
            Tu <strong className="text-pink-600">Masterclass Cejas & Pestañas</strong> está en camino. Te enviamos el acceso a tu email ahora mismo.
          </p>
        </div>
        ...
        <div className="bg-yellow-50 border-2 border-yellow-300 rounded-2xl p-4 text-left space-y-1">
          ...
          <p className="text-yellow-700 text-sm">
            Si no lo ves, <span className="font-bold">revisá la carpeta de SPAM</span>. A veces los emails automáticos caen ahí.
          </p>
        </div>
        ...
        <div className="bg-pink-50 border border-pink-100 rounded-2xl p-4 text-left space-y-2">
          ...
          <ul className="space-y-1">
            {[
              "13 módulos en video HD — acceso de por vida",
              "4 técnicas: cejas, lifting, pelo a pelo y volumen ruso",
              "Módulo de marketing para conseguir clientas",
            ].map((item) => (
